...
def get_weather2(region):
    ...
    key = config["weather_key"]
    region_url = "https://geoapi.qweather.com/v7/city/lookup?location={}&key={}&gzip=n".format(region, key)
    response = get(region_url, headers=headers).json()
    if response["code"] == "404":
        print("推送消息失败，请检查地区名是否有误！")
        os.system("pause")
...
if __name__ == "__main__":
    ...
    # 公众号推送消息
    for user in users:
        send_message(user, accessToken, region, weather, temp, wind_dir,air_quality, air_data, max_temperature, min_temperature, humidity1, note_ch, note_en)
    os.system("pause")
